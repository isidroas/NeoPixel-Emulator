    ...
    def draw_LED_matrix(self, width, height):
        for y in range(height):
            for x in range(width):
                ...
                self.color_sprites.append(pyglet.sprite.Sprite(img=pyglet.image.load(self.image_path2),batch=self.batch,x=x*35,y=self.height-34-(y*34)-5,group=self.color_group))
